...
% Thus, you enter the data as following:
% S = 500000000;
% Ug = 20000;
% percentageEg = 0.05;
% f = 60;
...
if ~isnumeric(S) || ~isnumeric(Ug) || ~isnumeric(percentageEg) || ~isnumeric(f) || ~isnumeric(Xsecond) ...
        || ~isnumeric(Xprime) || ~isnumeric(Xd) || ~isnumeric(Tsecond) || ~isnumeric(Tprime) || ~isnumeric(Ta)
    error('Invalid data, All the input values must be numeric. Please Try again')
end
